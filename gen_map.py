...
def gen_map(pif_id):
    ...
    fig.add_traces(
        go.Scattergeo(
            lon=[win_lon],
            lat=[win_lat],
            marker=dict(size=10, opacity=1, symbol="star", color="#ce1123"),
        )
    )
    ...
    fig.update_layout(
        showlegend=False,
        title=f"{pif.authorName}'s GeoPIF Results",
        geo=dict(
            projection=dict(type="kavrayskiy7", rotation_lon=win_lon),
            showland=True,
            landcolor="rgb(250, 250, 250)",
        ),
    )

    fig.write_image(file=f"pif_{pif_id}_result.png", width=1024, scale=3)

    imgur = ImgurClient(imgur_client_id, imgur_client_secret)
    image = imgur.upload_from_path(f"pif_{pif_id}_result.png")
    print(image["link"])
